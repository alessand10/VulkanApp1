...
void AppImage::bindToMemory(AppDeviceMemory *memory)
{
    VkDevice device = app->logicalDevice.get();
    VkImage image = this->get();
    VkDeviceMemory imageMemory = memory->get();

    THROW(vkBindImageMemory(device, image, imageMemory, 0U), "Failed to bind image to memory");
}
